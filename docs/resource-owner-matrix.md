...
|---|---|---|---|---|---|
| HTTP/1 keepalive connection | `KeepaliveConnection` isolate (`tina-http::keepalive`) | `Stop` closes the transport, isolate exits; `Maintain { now, max_idle }` closes an *idle* socket but keeps the slot | facade `InstalledKeepalivePool::close_and_drain(timeout)` (consuming; pool installed via `InstallKeepalivePool::install_keepalive_pool`) waits for leases to return, then `Stop`s each connection; raw-runtime consumers of `build_keepalive_pool` use the free `shutdown_keepalive_pool(CloseMode::Drain, ...)` | no force-close on the facade — it is drain-only; only the raw free `shutdown_keepalive_pool(CloseMode::Force, ...)` `Stop`s connections without waiting for leases | `KeepaliveCloseAndDrain` / `KeepalivePoolSettledReport` (facade), `KeepalivePoolShutdownReport` (raw helper); `KeepaliveOutcome::Maintained { closed_idle }` |
