...
scan() {
    find "$@" -type f -name '*.rs' \
        ...
        -exec perl -0777 -ne '
            ...
            my @rules = (
                [qr/\bArc::try_unwrap\s*\(\s*runtime\s*\)/, "Arc::try_unwrap(runtime) shutdown"],
                [qr/\b(?:runtime|rt|self\s*\.\s*runtime)\s*\.\s*shutdown\s*\(/, "unreported runtime.shutdown()"],
