...
Wraps an existing Specimen comparison and runs it under a series of
...
```bash
cargo run --manifest-path examples/specimen_mem_run/Cargo.toml -- \
  [comparison-manifest] [tiers-mb-comma-separated]
```

Defaults:
- `comparison-manifest` → `examples/specimen_real_io_chat/Cargo.toml`
...
```bash
cargo run --manifest-path examples/specimen_mem_run/Cargo.toml -- \
  examples/specimen_mini_keyspace/Cargo.toml 512,256,128
```
...
Same caveat as `specimen_cpu_run`: the existing comparisons assert a
