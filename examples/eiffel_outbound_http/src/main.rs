...
fn main() -> anyhow::Result<()> {
    ...
    match mode.as_str() {
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. usage: eiffel-outbound-http [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=eiffel_outbound_http side={} successful_get={} successful_post={} \
