...
fn main() -> ExitCode {
    ...
    match mode {
        ...
        "compare" => {
            ...
            println!(
                "\nboth client+server pairs walked the same scripted endpoint sequence \
                 and produced equivalent counter state."
            );
            ExitCode::SUCCESS
        }
        other => {
            eprintln!("usage: eiffel-outbound-http [tokio|tina|compare]");
