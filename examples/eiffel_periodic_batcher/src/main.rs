use eiffel_periodic_batcher::{Report, tina_impl, tokio_impl};
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. \
                 usage: eiffel-periodic-batcher [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=eiffel_periodic_batcher side={} items_seen={} \
