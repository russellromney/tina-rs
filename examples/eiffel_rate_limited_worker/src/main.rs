use eiffel_rate_limited_worker::{Report, tina_impl, tokio_impl};
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. \
                 usage: eiffel-rate-limited-worker [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=eiffel_rate_limited_worker side={} jobs_admitted={} \
