...
- a publish-back `send` to a known sink isolate (`tina-rpc` and
  `eiffel_dynamic_worker_pool` both do this);
...
```sh
cargo test --manifest-path examples/eiffel_owned_state_leak/Cargo.toml
```
...
```sh
cargo test --doc --manifest-path examples/eiffel_owned_state_leak/Cargo.toml
```
