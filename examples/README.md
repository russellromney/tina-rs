...
| `specimen_native_https` | Specimen | Native HTTPS/1.1 counter: a Tina `HttpsListener` server **and** a Tina `HttpClient` HTTPS client in one runtime (TLS rides the TCP rail, rustls sans-I/O) vs a hand-rolled `tokio + tokio-rustls` counterparty hit by a stdlib `rustls` client. |
