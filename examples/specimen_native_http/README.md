...
[`specimen_outbound_http`](../specimen_outbound_http/README.md) is the
*client* comparison. The complement.

## Run

```sh
cargo run --manifest-path examples/specimen_native_http/Cargo.toml -- both
cargo run --manifest-path examples/specimen_native_http/Cargo.toml -- tokio
cargo run --manifest-path examples/specimen_native_http/Cargo.toml -- tina
```
...
`StatefulRouter<Counter>` (the stateful router helper) —
