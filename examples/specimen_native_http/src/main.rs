use specimen_native_http::{Report, tina_impl, tokio_impl};
...
fn main() -> anyhow::Result<()> {
    ...
    match mode.as_str() {
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. usage: specimen-native-http [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=specimen_native_http side={} successful_get={} successful_post={} \
