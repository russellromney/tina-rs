...
| `system_tenant_rate_limiter` | Request-only per-tenant token buckets use owner-stamped time, fixed key capacity, typed hot/cold/table-full/closed outcomes, and live/simulator vocabulary parity. Run with `cargo test --manifest-path examples/systems/system_tenant_rate_limiter/Cargo.toml`. | `LocalSystem`, `RequestCall::now`, `RateLimitDecision`, move-only grants, bounded host inputs, deterministic simulator replay. |
