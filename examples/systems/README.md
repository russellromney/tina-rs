...
| `system_copied_service_path` | Canonical copied service skeleton: request entry, native protocol client, session control, durable restore, limits, load assertions, run capture, join-all, and select-next. Run with `cargo test --manifest-path examples/systems/system_copied_service_path/Cargo.toml`. | `RunCapture`, `WebSocketSessionMsg::AppControl`, load assertions, `CallJoinSet`, `CallSelectSet`, service topology. |
| `system_copied_service_path_companion` | Companion proof for the canonical skeleton. Run with `cargo test --manifest-path examples/systems/system_copied_service_path_companion/Cargo.toml`. | Compile/API proof for copied helpers and assertions. |
| `system_copied_service_path_smoke` | Cheap-model proof copied from the skeleton README. Run with `cargo test --manifest-path examples/systems/system_copied_service_path_smoke/Cargo.toml`. | Copied-path smoke only. |
