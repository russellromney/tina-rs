...
| `system_job_queue` | Bounded queue + N worker children, sync `Submit` parked in `PendingReplies`, `Cancel` for queued and in-flight jobs, retry budget that respawns the worker after `CallOutcome::Closed`. Run with `cargo test --manifest-path examples/systems/system_job_queue/Cargo.toml`. | `spawn_observed`, child lifecycle, `call_cancelable` + `PendingCallSet`, `PendingReplies`, `CallContext`, runtime-owned timers. |
