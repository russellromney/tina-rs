use specimen_outbound_http::{Report, tina_impl, tokio_impl};
...
fn main() -> anyhow::Result<()> {
    ...
    match mode.as_str() {
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. usage: specimen-outbound-http [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=specimen_outbound_http side={} successful_get={} successful_post={} \
