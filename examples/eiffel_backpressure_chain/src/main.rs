use eiffel_backpressure_chain::{Report, tina_impl, tokio_impl};
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. \
                 usage: eiffel-backpressure-chain [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=eiffel_backpressure_chain side={} successful={} \
         c_timed_out={} chain_dropped={} exit_clean={}",
        side,
        report.successful,
        report.c_timed_out,
        report.chain_dropped,
        report.exit_clean,
    );
}
