...
```sh
cargo run --manifest-path examples/specimen_mux_client/Cargo.toml -- both
cargo run --manifest-path examples/specimen_mux_client/Cargo.toml -- tokio
cargo run --manifest-path examples/specimen_mux_client/Cargo.toml -- tina
```

You'll see the same arrival order on both sides:

```
comparison=specimen_mux_client side=tokio arrival_order=[3, 2, 1]
comparison=specimen_mux_client side=tina  arrival_order=[3, 2, 1]
```
