...
```bash
cargo run --manifest-path examples/specimen_cpu_run/Cargo.toml -- \
...
```bash
cargo run --manifest-path examples/specimen_cpu_run/Cargo.toml -- \
  examples/specimen_mini_keyspace/Cargo.toml 4
```

## What this runner taught us

The current Specimen comparisons all produce a fixed, asserted output.
...
3. **What we cannot measure here.** Tina's "visible shedding" property
   ...
   `specimen_real_io_chat` has the closest approximation
