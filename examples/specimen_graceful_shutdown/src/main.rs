use specimen_graceful_shutdown::{Report, tina_impl, tokio_impl};
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio or tina (no `both` — see lib.rs). \
                 usage: specimen-graceful-shutdown [tokio|tina]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=specimen_graceful_shutdown side={} items_produced={} items_processed={} \
