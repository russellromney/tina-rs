use eiffel_dynamic_worker_pool::{Report, tina_impl, tokio_impl};
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. \
                 usage: eiffel-dynamic-worker-pool [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=eiffel_dynamic_worker_pool side={} results_collected={} \
