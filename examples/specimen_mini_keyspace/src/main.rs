use specimen_mini_keyspace::{Report, tina_impl, tokio_impl};
...
fn main() -> anyhow::Result<()> {
    ...
    match mode.as_str() {
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. usage: specimen-mini-keyspace [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=specimen_mini_keyspace side={} ok={} values={} misses={} deleted={}",
