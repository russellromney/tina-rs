...
```sh
cargo run --manifest-path examples/specimen_mini_keyspace/Cargo.toml -- both
cargo run --manifest-path examples/specimen_mini_keyspace/Cargo.toml -- tokio
cargo run --manifest-path examples/specimen_mini_keyspace/Cargo.toml -- tina
```

You'll see the same counts on both sides:

```
comparison=specimen_mini_keyspace side=tokio ok=1 values=1 misses=2 deleted=1
comparison=specimen_mini_keyspace side=tina  ok=1 values=1 misses=2 deleted=1
```
