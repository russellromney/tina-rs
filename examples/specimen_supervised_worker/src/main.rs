use specimen_supervised_worker::{Report, tina_impl, tokio_impl};
...
fn main() -> anyhow::Result<()> {
    ...
    match mode.as_str() {
        ...
        other => {
            anyhow::bail!(
                "unknown mode {other:?}; expected tokio, tina, or both. usage: specimen-supervised-worker [tokio|tina|both]"
            );
        }
    }
    Ok(())
...
fn print_side(side: &str, report: Report) {
    println!(
        "comparison=specimen_supervised_worker side={} processed={} poisoned={} restarts={} exit_clean={}",
