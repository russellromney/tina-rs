...
```sh
cargo run --manifest-path examples/eiffel_real_io_chat/Cargo.toml -- both
cargo run --manifest-path examples/eiffel_real_io_chat/Cargo.toml -- tokio
cargo run --manifest-path examples/eiffel_real_io_chat/Cargo.toml -- tina
cargo run --manifest-path examples/eiffel_real_io_chat/Cargo.toml -- both 256
```

You'll see one line per side:

```
comparison=eiffel_real_io_chat side=tokio burst=64 accepted=64 full=0 closed=0 delivered=1 buffered=63
comparison=eiffel_real_io_chat side=tina  burst=64 accepted=1  full=63 closed=0 delivered=1 buffered=0
```
