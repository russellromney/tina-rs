...
/// Result of [`DynamoWorker::install`] or [`DynamoWorker::install_local`].
...
impl<S: Shard + 'static> Drop for DynamoWorker<S> {
    fn drop(&mut self) {
        self.closed.store(true, Ordering::Release);
    }
}
...
impl<S: Shard + Send + 'static> DynamoWorker<S> {
    fn install_via(
        ...
        register: impl FnOnce(
            Self,
            usize,
        ) -> Result<Address<DynamoMsg, DynamoResult>, ThreadedRuntimeError>,
        ...
        let address = register(worker, cap).map_err(DynamoInstallError::Register)?;
...
/// Local-system first form of [`install_dynamodb`].
pub fn install_dynamodb_local<S, F>(
    system: &LocalSystem<S, F>,
    config: DynamoConfig,
) -> Result<InstalledDynamoBridge<S>, DynamoInstallError>
where
    ...
    F: MailboxFactory + Send + 'static,
{
    DynamoWorker::<S>::install_local(system, config)
}
...
mod install_tests {
    ...
    fn rejected_install_drops_the_fully_built_worker() {
        for expected in [
            ...
            let result = DynamoWorker::<SingleShard>::install_via(
            ...
            assert!(
                matches!(result, Err(DynamoInstallError::Register(error)) if error == expected)
            );
            let closer = retained
                .lock()
