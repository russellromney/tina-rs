...
/// Snapshot of SNS bridge counters.
...
pub struct SnsMetrics {
    ...
    /// Rejected at admission because attributes exceeded the cap.
    pub attributes_too_large: u64,
    /// Bridge per-operation timeouts.
    pub timeouts: u64,
    /// SDK futures that completed successfully.
    pub responses: u64,
    /// SNS rejected the parameters.
    pub invalid_parameter: u64,
    /// SNS topic/endpoint not found.
    pub not_found: u64,
    /// SNS denied the caller.
    pub access_denied: u64,
    /// SNS throttled the caller.
    pub throttled: u64,
...
impl SnsMetricsInner {
    pub(crate) fn snapshot(&self) -> SnsMetrics {
        SnsMetrics {
            ...
            attributes_too_large: self.attributes_too_large.load(Ordering::Relaxed),
...
#[derive(Debug, Clone)]
pub struct SnsMetricsHandle {
    pub(crate) inner: Arc<SnsMetricsInner>,
    pub(crate) capacity: usize,
}

impl SnsMetricsHandle {
    /// Returns a fresh snapshot.
    pub fn snapshot(&self) -> SnsMetrics {
        self.inner.snapshot()
    }
