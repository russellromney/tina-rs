# Phase 059: Specimen Actionable Ergonomics

## Goal

Turn the current Specimen findings into Tina product improvements.
...
1. **Typed Isolate Result Waiters** — *landed*
   ...
   - update at least two specimens that currently carry side-channel
     final data (target: `specimen_outbound_fetch`,
...
2. **Continuation And Pipeline Sugar** — *landed (first form)*
   ...
   - `specimen_outbound_fetch` (TCP client) and `specimen_persistent_counter`
...
3. **TCP Loop Helpers** — *landed (client-side first form)*
   ...
   trace event. `specimen_outbound_fetch` converted; canonical pattern
   ...
   - update `specimen_outbound_fetch` or another TCP specimen.
...
5. **Bounded Host Send Helpers**
   ...
   - replace at least one manual `IngressFull` retry loop in a specimen
     or runtime test.
...
6. **Tiny Native HTTP Router** — *landed*
   ...
   - `specimen_native_http`'s `Counter::handle` converted from a
...
7. **Bridge Specimen Rewrite** — *landed*
   ...
   Shipped: `specimen_axum_counter` and `specimen_ws_room` rewritten to
...
8. **RPC Service Topology** — *deferred (runtime prerequisite)*
   ...
   *multiple* pending `IsolateCall` continuations simultaneously: one
   ...
   - Specimen RPC follow-up if the example teaches something new.
...
9. **Pressure Report Convention** — *landed*
   ...
   - `specimen_real_io_chat` opts in and prints the line per side;
   - `specimen_cpu_run` captures the target's stdout, intercepts
   ...
   - update one pressure-capable specimen plus `specimen_cpu_run` or
     `specimen_mem_run`;
...
- At least three specimens get shorter or clearer without losing
...
- The next Specimen pressure round can focus on behavior under load, not old
  setup ceremony.
