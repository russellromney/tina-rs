...
one Eiffel example as proof; the closeout commit fans the primitives
...
- **Sequencing.** "One end-to-end slice first" against
  `eiffel_mini_keyspace` proved the harvest pattern early
...
- **Rock 3 — Stable trace fingerprint.** `RuntimeEvent::stable_hash`
  ...
  `format!("{event:?}").hash(...)` pattern in `eiffel_replay_dst` is
...
- **Rock 6 — Sequence + TCP loop docs.** `tina::sequence(...)` is
  ...
  `eiffel_mux_client`. `docs/tcp-loops.md` ships canonical write-all
...
- **Rock 7 — Bridge lifecycle.** `BridgeHost::drain_and_shutdown`,
  ...
  from `eiffel_axum_counter` and `eiffel_ws_room`.
...
Every Eiffel comparison except the runner shells (`eiffel_cpu_run`,
`eiffel_mem_run`) was rewritten to use the new primitives:
...
| ------------------------- | -----: | ----: | ---: |
| eiffel_axum_counter       |    188 |   119 |  −69 |
| eiffel_graceful_shutdown  |    285 |   221 |  −64 |
| eiffel_mini_keyspace      |    356 |   260 |  −96 |
| eiffel_mux_client         |    219 |   143 |  −76 |
| eiffel_outbound_fetch     |    246 |   175 |  −71 |
| eiffel_persistent_counter |    366 |   303 |  −63 |
| eiffel_real_io_chat       |    359 |   283 |  −76 |
| eiffel_replay_dst         |    148 |   121 |  −27 |
| eiffel_supervised_worker  |    313 |   246 |  −67 |
| eiffel_ws_room            |    211 |   143 |  −68 |
...
- All ten primary Eiffel `compare` modes run and produce equivalent
  Tokio / Tina outcomes.
