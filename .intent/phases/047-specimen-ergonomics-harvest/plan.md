...
Specimen found the pain. 047 makes the pain smaller.
...
At closeout, Specimen examples should delete boilerplate, side channels, and
...
- Repeated Specimen pain earns helpers. One-off weirdness gets documented first.
...
- Rerun Specimen after changes. The proof is deleted workaround code.
...
1. **Default Mailbox Factory**
   ...
   - update Specimen examples to use it;
...
4. **Host Observation Handles**
   ...
   handle shape that deletes Specimen side channels:
   ...
   - remove at least one trace-polling loop from a specimen;
   - update the relevant Specimen README to say which side channel disappeared;
...
- `cargo check --manifest-path ...` for each `examples/specimen_*`.
...
- Specimen examples updated to use the new primitives where they apply.
...
- Specimen examples have less boilerplate and fewer side channels.
...
- Tina is not yet a native HTTP framework, not yet `io_uring`, and not yet a
  broad Tokio replacement. It is simply less sharp in the places Specimen already
