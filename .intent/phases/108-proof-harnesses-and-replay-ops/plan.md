...
Define which harnesses are fast PR gates and which are scheduled/soak gates.
Keep local commands copyable.
...
- smoke test
- load or bad-peer proof if networked
- findings section
- roadmap pointer for discovered rough bits

The harness should make this easy enough for cheap model sessions to use.
...
A weird production-ish failure can become a saved case that a future session can
replay, shrink, and fix without guessing.
