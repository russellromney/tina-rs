...
- Done:
  - Round 2 findings 1-7 harvested from specimens.
  - Rock 0: re-read of `specimen_rate_limited_worker`,
    `specimen_retrying_outbound_http`, `specimen_sharded_fanout_read`. The
  ...
  - Rock 6: `ReqwestOutcomeExt::classify` with typed reasons
    ...
    `UpstreamClient { status }`). `specimen_retrying_outbound_http`
...
Turn the second Specimen pain harvest into Tina primitives.
...
- Examples are specimens. If a primitive lands, rewrite the specimen so
...
### Rock 0: Wait For The Next Specimen Batch

Before implementation, read the next three specimens and their README
notes.
...
Do not churn APIs before this read. Specimen is the scout.
...
- update `specimen_sharded_fanout_read` and `specimen_sharded_keyspace` to delete
  host polling.
...
- update `specimen_rate_limited_worker` to remove observer atomics/barrier.
...
- update `specimen_rate_limited_worker` only if the helper is clearer than the
...
- update `specimen_retrying_outbound_http` to delete the six-arm local match.
...
- update `specimen_webhook_publisher` only if this makes the example clearer;
...
- one closed target;
- aggregate timeout;
- update `specimen_sharded_fanout_read`.
...
1. Rock 0: wait for next Specimen batch.
...
- Every affected Specimen specimen gets rewritten after the primitive lands.
...
- Round 2 current findings are either implemented, explicitly deferred, or
  superseded by findings from the next Specimen batch.
- At least three Specimen examples delete visible ceremony because of this
  phase.
