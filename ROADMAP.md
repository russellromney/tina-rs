...
| **065 observability first form** | Make Tina's visible facts easier to consume: tracing/export shape, pressure summaries, bridge/runtime event naming, and operator-facing reports. Keep it focused; do not smuggle in cancellation or deadline semantics beyond naming the events future phases need. |
| **066 cancellation and deadline model** | Own the real cancellation primitive. `cancel_call(handle)` is the load-bearing shape; isolate `Stop` remains domain message; already-accepted external work may still finish and late replies must be visible. Deadline belongs here only after the clock story is honest; otherwise defer deadline to simulator-clock work. |
| **067 bounded pool vocabulary** | Give worker/resource pools one Tina-shaped language: acquire, lease, release/retire, drain/force close, FIFO bounded waiters, metrics, and explicit `Full`/`Closed`/`Timeout`. Cancellation never releases a lease by magic. |
| **068 production HTTP body/TLS maturity** | Mature the native HTTP path after the bridge/database work: production body streaming ergonomics, HTTPS/TLS surface, larger real-service examples, and honest memory/backpressure limits. No chunk/body helper may hide where bytes buffer or who owns backpressure. |
| **069 DST and replay usability** | Tighten replay ergonomics, saved seeds, trace fingerprints, and the runtime/simulator clock story. Any serious `Deadline` API that participates in replay should land here or after this clock model exists; a live-only helper must say it has no simulator claim. |
