...
| **060 Betelgeuse adapter safety** | Keep Betelgeuse as Tina's canonical portable live backend and harden the adapter boundary. Add driver-contract docs, closing-resource tombstones for pending TCP/listener/file work, mean close-cancel lifetime tests, hostile fake-backend seams where useful, and a completion-slot allocation plan. This is not North Sea and not a replacement. It is seatbelt work: Tina must keep completion slots and backend-referenced resources alive until backend release while user-visible close still settles pending calls as `ResourceClosed`. |
