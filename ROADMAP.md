...
| **Voyager deterministic simulation** | Planning bucket with the first reviewed slice delivered in `.intent/phases/016-voyager-virtual-time-and-replay/`. Shipped so far: `tina-sim`, a single-shard virtual-time simulator for the shipped `Sleep { after }` / `TimerFired` contract, deterministic replay artifacts, direct timer-semantics proofs, and a simulator-backed retry/backoff proof workload. Remaining Voyager work still includes PRNG policy, faults, checkers, broader I/O simulation, injected-bug proof, and later multi-slice expansion. |
...
- First reviewed Voyager foothold is now landed: `tina-sim` exists as a
  single-shard virtual-time simulator for the shipped timer contract
  (`Sleep { after }` / `TimerFired`), captures replay artifacts, and
  proves a retry/backoff workload under virtual time. TCP simulation,
  failure injection, checker infrastructure, and broader seeded
  perturbation still follow in later Voyager slices.
