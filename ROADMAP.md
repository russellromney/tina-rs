...
| **125 Runtime supervision and fairness** | Post-122 core wave. First implementation wave shipped: non-panic typed failure (`Effect::Fail`), explicit supervised shutdown (`Effect::StopChildren`), `SupervisorReport`, and `FairnessReport`/`StarvationWarning` with hot/quiet/timer proofs. Still open: local multi-shard child ownership. Plan + findings: `.intent/phases/125-supervision-failure-domains/`. |
