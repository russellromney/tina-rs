...
- Recent core capability tranche: capacity modeling round 2, bounded
  ...
  surfaces, timer helper vocabulary, and native HTTP/2 first form. These are
  recorded in `CHANGELOG.md`; the next work is WebSocket, live-replay
  expansion, gRPC, and production service layers.
...
| **083 production service layers** | Package the copied service patterns into one real skeleton: HTTP routing + state isolate + DB pool + outbound keepalive + health/readiness + graceful shutdown + tracing + capacity assertions + DST/live-replay hook. Not a framework on top of Tina; a "real service skeleton" proving an LLM can assemble the layers safely. Plan: `.intent/phases/083-production-service-layers/plan.md`. |
| **095 call-context defer ergonomics** | Replace the tiny `reply_with_current_request(...)` idea with a caller-rooted request workflow: `call_ctx.defer(work).reply(Msg::Done)` as the blessed multi-turn path, `work.then(...)` for ordinary continuations, and the expanded `into_request_context()` form still documented once. No hidden caller carry, no generic `Context`, no fake async stack. Plan: `.intent/phases/095-call-context-defer-ergonomics/plan.md`. |
| **094 WebSocket usable server** | Follow 087's first form with the user-shaped server layer: public session handles, bounded room/fanout, slow-peer policy, TCP/TLS e2e proof, copyable docs, and a real room specimen. Plan: `.intent/phases/094-websocket-usable-server/plan.md`. |
| **095 HTTP/2 streaming substrate** | Turn the shipped HTTP/2 first form from buffered unary transport into a server-side bidirectional streaming DATA/trailer substrate: response streaming, request streaming, real flow-control pressure, peer-reset cancellation, bounded queues/bytes, and regression proof for buffered HTTP/2 plus unary gRPC. This is the substrate required before real gRPC streaming. Plan: `.intent/phases/095-http2-streaming-substrate/plan.md`. |
| **096 finish native gRPC streaming** | Stack on 095 to finish native gRPC. In progress on PR #85: server-streaming, true incremental client-streaming routes, and a reusable `GrpcRequestStream<T>::next` pull handle now ride the native HTTP/2 h2c server with typed status/trailers, early-final behavior, tonic h2c smoke interop, and live tests. Still open: true bidirectional streaming, automated grpcurl interop, production pooled Tina gRPC client, and TLS ALPN. Plan: `.intent/phases/096-finish-native-grpc-streaming/plan.md`. |
| **057 native gRPC service stack** | Unary server shipped on native HTTP/2 h2c with `prost`, typed `GrpcStatus`, trailers, caps, timeout/cancel/status mapping, and a tiny h2c specimen helper. PR #85 now extends this with server-streaming, incremental client-streaming, and a `GrpcRequestStream<T>::next` pull handle on native HTTP/2; further gRPC progress still stacks behind completing **096 finish native gRPC streaming** for bidi and production claims. Production client, true bidi, automated grpcurl interop, and TLS ALPN remain follow-up work until proven. Plan: `.intent/phases/057-native-grpc-service-stack/plan.md`. |
| **096 WebSocket production replacement** | One integrated phase after 094 for the full "replace a production WebSocket stack with Tina" claim: Autobahn/compliance classification, required browser `ws://` and `wss://`, upgrade admission/auth/subprotocol hooks, production realtime-room lifecycle, load/soak proof, native-client decision, live trace to simulator replay, and production observability/docs. Plan: `.intent/phases/096-websocket-production-replacement/plan.md`. |
