...
| Backpressure policies | Shipped: admission/rate/concurrency policies, `FullHandling`, and typed pressure actions. Remaining: compose multiple budgets atomically, reduce parked-caller charge ceremony, and prove retry/degrade policies in more full-service specimens. | Services choose pressure behavior at call sites without losing `Full`/`Closed`/`Timeout` truth. |
