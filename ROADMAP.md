...
| **101 typed config and budget manifests** | Broad compile-time/config-safety pass after 100, not a tiny polish slice. Build typed builders/manifests for the service knobs users copy most: mailbox caps, pool caps, body caps, bridge in-flight caps, deadlines, retry budgets, shared capacity scopes, and startup config. Runtime validation remains required for env/file config, but hand-built Rust config should make missing/zero/impossible states hard to write. Output should include a printable/diffable service budget manifest and e2e proof that config used by live services, capacity reports, and replay cases is the same config. |
| **102 protocol typestate hardening** | Broad internal protocol safety pass after the service-model split. Use private typestate/state-token shapes in `tina-http` where they eliminate real protocol bugs: HTTP/2 stream lifecycle, request/response DATA/trailers ordering, WebSocket close handshake, gRPC final-status ownership, and possibly client/server body stream states. This is important core hardening, not optional garnish, but it should stay private unless a public type earns its keep. User-facing service code must remain simple while impossible protocol transitions become unrepresentable inside the implementation. |
