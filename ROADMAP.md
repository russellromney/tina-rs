...
- Recent core capability tranche: capacity modeling round 2, bounded
  ...
  recorded in `CHANGELOG.md`; the next work is request-context defer polish,
  HTTP/2/gRPC streaming substrate, production WebSocket replacement, and
...
| **095 call-context defer ergonomics** | Replace the tiny `reply_with_current_request(...)` idea with a caller-rooted request workflow: `call_ctx.defer(work).reply(Msg::Done)` as the blessed multi-turn path, `call_ctx.defer_cancelable(call_cancelable(...))` for explicit cancelable work, `work.then(...)` for ordinary continuations, and the expanded `into_request_context()` form still documented once. No hidden caller carry, no generic `Context`, no fake async stack. Keep `call_with_handle(...)` only as deprecated compatibility and scrub it in a later cleanup. Plan: `.intent/phases/095-call-context-defer-ergonomics/plan.md`. |
| **097 cancelable deferred admission** | Make `call_ctx.defer_cancelable(...)` safe to copy. First prove the pattern in one real specimen with local bounded storage: store the pending token before returning the child effect; on `Full`/duplicate, recover the caller with `pending.into_request_context()` and do not dispatch the effect. Then extract `PendingCancelableCallSet` only if the shape is reusable. Plan: `.intent/phases/097-cancelable-deferred-admission/plan.md`. |
...
| **096 finish native gRPC streaming** | Stack on 095 to finish native gRPC. First server-streaming and client-streaming now ride the native HTTP/2 h2c server with typed status/trailers and live tests. Still open: true bidirectional streaming, tonic/grpcurl interop scripts, production pooled Tina gRPC client, and TLS ALPN. Plan: `.intent/phases/096-finish-native-grpc-streaming/plan.md`. |
...
| **097 WebSocket production hardening stretch** | One stretched but reviewable server-hardening phase: browser `wss://`, Origin/auth/admission, fuller subprotocol negotiation, close/heartbeat policy, per-session and room reports, production room lifecycle, load/soak proof, and documented browser/`tungstenite` client story. This is the "usable in anger for bounded realtime rooms" gate, not the final standards/replay replacement gate. Plan: `.intent/phases/097-websocket-production-hardening-stretch/plan.md`. |
