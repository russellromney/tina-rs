...
| Phase | Purpose |
|---|---|
| **San production-readiness rails** | Build the first serious "can a real Tokio/Glommio-shaped service be moved to Tina?" gate: executable capability matrix, user-perspective service gauntlet, DST gauntlet, live thread-per-core pressure, bridge/runtime cancellation rails, Tina/Tokio/Glommio behavior comparisons, benchmark skeleton, CI/release rails, and sharp non-claims. This is not a docs/demo phase; it is the next production-readiness test wall. |
