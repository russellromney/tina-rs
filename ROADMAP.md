...
| **119 Production resource and data maturity** | Wave A, after native protocol clients. Resource half shipped: `ResourceLifetime`/`ResourceHealth`/`RetireReason` pool policy, `WorkerPool` idle/max-age maintenance sweep + owner refill, `PoolShutdownReport`, HTTP/1 keepalive idle retirement, and a checked-in resource owner matrix (`docs/resource-owner-matrix.md`); DB bridge pressure confirmed aligned without faking SQLx internals. Durable half (snapshot/journal restore service, recovery report, corrupt/torn-tail specimens) deferred to Phase 126 to avoid colliding names. Plan + findings: `.intent/phases/119-production-resource-data-maturity/`. |
...
| **125 Runtime supervision and fairness** | Post-122 core wave. Shipped: non-panic typed failure (`Effect::Fail`), explicit supervised shutdown (`Effect::StopChildren`), `SupervisorReport`, `FairnessReport`/`StarvationWarning` with hot/quiet/timer proofs, and cross-shard observed spawn (`spawn_observed(child).on_shard(shard)`, proven live + sim). Still open: cross-shard stop/restart/address-change. Plan + findings: `.intent/phases/125-supervision-failure-domains/`. |
