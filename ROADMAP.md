...
| **102 host control ergonomics** | Combine the two host-control findings from systems: add `ThreadedMultiShardRuntime::call_blocking_on(shard, addr, msg, timeout)` for honest sharded smoke tests, and add `ThreadedShutdownHandle` so host code can request/wait runtime shutdown without `Arc::try_unwrap(runtime)`. Tests must prove normal call outcomes, timeout/stale/wrong-shard behavior, cached terminal shutdown report truth, and one real sharded + one shutdown-heavy specimen migration. Plan: `.intent/phases/102-host-control-ergonomics/plan.md`. |
