...
| **125 Supervision and failure domains** | Post-122 core wave. Cross-shard child ownership or typed rejection, parent-stop cleanup, replacement address refresh, failed-shard ingress truth, non-panic failure policy, and supervisor reports. Plan outline: `.intent/phases/125-supervision-failure-domains/plan.md`. |
| **126 Durable work and restartable state** | Post-122 core wave. Durable work/outbox first form, append-before-apply helper, bounded replay, restart recovery reports, corrupt/truncated/uncertain-tail outcomes, and durable service specimens. Plan outline: `.intent/phases/126-durable-work-restartable-state/plan.md`. |
| **127 Unified session and stream lifecycle** | Post-122 core wave. Shared lifecycle vocabulary and close/cancel/drain/report behavior for TCP sessions, HTTP/2 streams, gRPC streams, and WebSocket sessions. Plan outline: `.intent/phases/127-session-stream-lifecycle/plan.md`. |
| **128 Native protocol completion** | Post-122 core wave. Native WebSocket client, HTTP/2 TLS ALPN, mTLS if it fits the rustls shape, gRPC client polish, pooled HTTP/2/gRPC clients, and client-side protocol facts. Plan outline: `.intent/phases/128-native-protocol-completion/plan.md`. |
| **129 Runtime fairness as capability** | Post-122 core wave. Starvation-visible runtime/service reports, hot actor/session/timer/remote-edge progress proofs, constrained load profiles, and fairness counters that compose with pressure reports. Plan outline: `.intent/phases/129-runtime-fairness-capability/plan.md`. |
| **130 Storage, file, and IPC completion** | Post-122 core wave. Production completion for file streaming, rename-commit/directory-sync truth, Unix socket rails, framed local IPC, lifecycle/pressure parity, and local sidecar specimens. Plan outline: `.intent/phases/130-storage-file-ipc-completion/plan.md`. |
