...
| App/service ergonomics | Specimen work has turned repeated example pain into typed result waiters, bounded observation handles, reply aliases, TCP/Unix loop helpers, pressure summaries, HTTP router sugar, sharded placement/table helpers, deferred replies, `CallContext` reply obligation, `RequestContext` multi-turn replies, typed child refs via `spawn_observed`, cancellation handles, deadlines, pending-call sets, bounded pools, host-burst helpers, single-call gates, reqwest/DB classifiers, capacity reports, all-or-nothing shared-capacity reservations, weighted/shared body capacity, timer helper state, `RecurringTick`, `LocalPermitGate`, `DrainState`, `FullHandling`, register-and-bootstrap helpers, `PendingCancelableCallSet`, bounded first-success `CallGroup` with cancelable branch-start sugar, threaded shutdown handles, and host `call_blocking` scripts where appropriate. | Cross-isolate paired registration, generic scatter/gather happy-path helpers, host-side scenario/test ergonomics, join-all/stream-select helpers, natural-key keyed-wait helpers, framed writer helpers, mid-run observation/projection helpers, and more real-world specimens remain future work. |
...
starts with ergonomics, load/fairness, session lifecycle, and live replay work
below.
...
| **121 Fairness and load behavior** | Scheduler/session fairness proofs, timer fairness under hot load, protocol session fairness, remote inbound drain fairness, lag-ish reports, soak/load harnesses, and CPU/memory constrained runs. Fold in what `FairnessReport` can prove today, then add only the missing instrumentation needed for user-visible load truth. Plan outline: `.intent/phases/121-fairness-load-behavior/plan.md`. |
...
| **128 Live trace to sim replay expansion** | Post-121/127 superpower pass. Turn live weirdness into a saved simulator case: bounded capture builder, fact extractors, unsupported-fact fail-closed behavior, proof-harness integration, live-derived shrinking, and one real system bug-box workflow. Plan outline: `.intent/phases/128-live-trace-to-sim-replay-expansion/plan.md`. |
...
| Protocol and client breadth | Shipped: HTTP/2 server/client, gRPC server/client streaming modes, WebSocket server, AWS bridge breadth, and protocol facts. Remaining: native broad WebSocket client, HTTP/2 mTLS, gRPC reflection/interceptors/load balancing, client-side session lifecycle polish, and protocol-byte replay where facts are not enough. | Common Tokio protocol workloads have native Tina paths instead of falling back to Tokio bridges at the first real boundary, without hiding stream pressure. |
