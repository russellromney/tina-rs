...
| **101 mailbox-first service ergonomics** | Build the helpers repeatedly pulled by system specimens: clearer non-cancelable deferred work to self, recurring tick/missed-tick service patterns, isolate-local in-flight permits, explicit graceful drain state, and possibly startup effects if the design preserves registration/restart truth. No fake async, no hidden mutation, no hidden retry. Plan: `.intent/phases/101-mailbox-first-service-ergonomics/plan.md`. |
