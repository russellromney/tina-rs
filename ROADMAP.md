...
| **087 WebSocket first form** | Native WebSocket over Tina-owned HTTP/TCP/TLS rails. Use `tungstenite` core for frame parsing, but Tina owns session isolates, ping/pong, close handshake, slow-reader pressure, inbound/outbound caps, cancellation, trace, and specimens. First form should cover server rooms and one client shape, not a broad web framework. |
| **088 AWS bridge first form** | Bounded bridge for the production AWS shapes real services hit first: likely S3 object get/put/head and SQS send/receive/delete before broad SDK coverage. Preserve bridge truth: explicit caps, caller-owned retry/idempotency, shutdown, late-result/cancel truth, metrics, and no hidden queue. |
| **089 live trace to sim replay workflow** | Turn DST into an ops workflow: capture live inputs/resource completions/topology facts into a replayable history, project into `tina-sim`, compare traces, and shrink the bad case. This builds on 069's saved replay cases but starts from live evidence. |
