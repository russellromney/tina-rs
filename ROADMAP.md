...
| **096 finish native gRPC streaming** | Stack on 095 to finish native gRPC: server-streaming, client-streaming, bidirectional streaming, typed status/deadline/cancel/pressure for every mode, specimen coverage, and h2c interop tests where practical. Production pooled gRPC client may land here only if a real HTTP/2 client state machine exists; otherwise it gets its own phase. Plan: `.intent/phases/096-finish-native-grpc-streaming/plan.md`. |
| **057 native gRPC service stack** | Unary server shipped on native HTTP/2 h2c with `prost`, typed `GrpcStatus`, trailers, caps, timeout/cancel/status mapping, and a tiny h2c specimen helper. Further gRPC progress now stacks behind **095 HTTP/2 streaming substrate** and **096 finish native gRPC streaming**; production client and broad TLS ALPN interop may still require follow-up phases. Plan: `.intent/phases/057-native-grpc-service-stack/plan.md`. |
