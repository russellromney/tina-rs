...
| **096 finish native gRPC streaming** | Stack on 095 to finish native gRPC. In progress on PR #85: first server-streaming and client-streaming routes now ride the native HTTP/2 h2c server with typed status/trailers and live tests. Still open: true bidirectional streaming, tonic/grpcurl interop scripts, production pooled Tina gRPC client, and TLS ALPN. Plan: `.intent/phases/096-finish-native-grpc-streaming/plan.md`. |
| **057 native gRPC service stack** | Unary server shipped on native HTTP/2 h2c with `prost`, typed `GrpcStatus`, trailers, caps, timeout/cancel/status mapping, and a tiny h2c specimen helper. PR #85 now extends this with first server-streaming/client-streaming on native HTTP/2; further gRPC progress still stacks behind completing **095 HTTP/2 streaming substrate** and **096 finish native gRPC streaming**. Production client, true bidi, tonic/grpcurl interop, and TLS ALPN remain follow-up work until proven. Plan: `.intent/phases/057-native-grpc-service-stack/plan.md`. |
