...
| **095 HTTP/2 streaming substrate** | Turn the shipped HTTP/2 first form from buffered unary transport into a streaming DATA/trailer substrate: response streaming first, request streaming if clean, real flow-control pressure, peer-reset cancellation, bounded queues/bytes, and regression proof for buffered HTTP/2 plus unary gRPC. This blocks honest gRPC server-streaming; client-streaming/bidi also need request streaming, and production gRPC clients additionally need a future HTTP/2 client state-machine phase. Plan: `.intent/phases/095-http2-streaming-substrate/plan.md`. |
| **057 native gRPC service stack** | Unary server shipped on native HTTP/2 h2c with `prost`, typed `GrpcStatus`, trailers, caps, timeout/cancel/status mapping, and a tiny h2c specimen helper. Further gRPC progress is intentionally stuck behind **095 HTTP/2 streaming substrate** for server/client/bidi streaming and behind future HTTP/2 client/TLS ALPN work for production client and broad interop. Plan: `.intent/phases/057-native-grpc-service-stack/plan.md`. |
...
| Protocol and client breadth | WebSocket, unary gRPC, broader AWS surfaces, and broader RPC hardening, using sync codecs where possible while Tina owns I/O/backpressure. HTTP/2 streaming substrate is the next blocking layer before honest gRPC server-streaming; request streaming and a later HTTP/2 client state machine are still required before client-streaming/bidi or production pooled gRPC clients. | Common Tokio protocol workloads have native Tina paths instead of falling back to Tokio bridges at the first real boundary, without hiding stream pressure. |
