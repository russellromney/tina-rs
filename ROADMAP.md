...
| **114 Post-113 framework ergonomics** | Digest phases 110-113 into one copied service path before the next architecture/core wave: skeleton, prelude/import guidance, noun guide, system rewrites, stale findings cleanup, and cheap-model proof. Plan outline: `.intent/phases/114-post-113-framework-ergonomics/plan.md`. |
| **115 Core / ecosystem reorg** | Draw the core-vs-batteries boundary before Wave A adds more official batteries. Define learn-core vs choose-batteries docs, official battery rules, prelude/import tiers, public-hook gaps, and dependency-layer rules without doing a cosmetic mass move. Plan outline: `.intent/phases/115-core-ecosystem-reorg/plan.md`. |
...
| **118 Production resource and data maturity** | Wave A. Pool idle eviction/max lifetime/health/retire rules, pool shutdown reports, pooled DB/HTTP/gRPC clients, snapshot/journal restore patterns, and crash/corrupt-tail recovery specimens. Plan outline: `.intent/phases/118-production-resource-data-maturity/plan.md`. |
| **119 Post-Wave-A ergonomics** | Digest Wave A into the copied service skeleton: outbound protocol clients, file/codec/local IPC, mature pools, durable restore, updated docs/findings, and cheap-model proof. Plan outline: `.intent/phases/119-post-wave-a-ergonomics/plan.md`. |
| **120 Fairness and load behavior** | Wave B. Scheduler/session fairness proofs, timer fairness under hot load, protocol session fairness, remote inbound drain fairness, lag-ish reports, soak/load harnesses, and CPU/memory constrained runs. Plan outline: `.intent/phases/120-fairness-load-behavior/plan.md`. |
| **121 Admission and rate policy** | Wave B. Explicit service pressure policies: concurrency/per-key/per-user/rate limits, bounded wait, shed/degrade/close, retry budget, typed outcomes, capacity/service-report integration, and edge-service specimens. Plan outline: `.intent/phases/121-admission-rate-policy/plan.md`. |
