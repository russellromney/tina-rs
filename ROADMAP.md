...
| Single-shard runtime delivery | `tina-runtime-current` has deterministic trace IDs and causal links, registration-order stepping, local send dispatch, local spawn dispatch, typed ingress, stop-and-abandon, panic capture, address generations, runtime-owned parent-child lineage, restartable child records, and generated-history property tests. | `RestartChildren` is still observed-only; no actual supervisor restart mechanism exists yet. The generated-history model is bounded and does not prove arbitrary user programs. |
...
- Build supervision on stored runtime state, not trace reconstruction. Parent-child lineage already exists; the remaining supervision work needs restartable child records and an address liveness model that can represent stale post-restart targets.
...
- The remaining supervisor tests should prove actual runtime behavior:
  ...
  - `RestartChildren` consumes stored lineage/restart records instead of reconstructing parenthood from the trace
