...
| **Threaded multishard host calls** | Small host-ergonomics phase pulled by `system_cache_with_fill`, `system_job_queue`, and `system_session_auth`: add `ThreadedMultiShardRuntime::call_blocking_on(shard, addr, msg, timeout)` or equivalent typed host-call helper, using the existing host driver shape on the worker that owns the shard. Tests should prove success, timeout, closed/stale address, wrong-shard rejection, and one real system/specimen using actual multi-shard placement instead of in-isolate sharding. |
...
| **Runtime shutdown request handle** | Small lifecycle/host ergonomics phase pulled by `system_metrics_shipper`: provide a copied shutdown path that does not require sole `Arc` ownership or `Arc::try_unwrap(runtime)`. A host should be able to request stop/drain, wait for terminal reports, and observe timeout/closed outcomes without tearing ownership apart. This must not hide service-specific drain order; it should trigger/observe explicit shutdown messages or runtime-owned shutdown facts. |
