...
| **072 deadline and pending call set** | Finish the two pieces of vocabulary deferred from 066's first-form cancellation: an absolute `Deadline` value that flows A→B→C without re-doing timeout arithmetic, and a bounded `PendingCallSet` for storing many caller-owned `CallHandle`s in isolate state with explicit insert/remove/drain-cancel and typed `Full`/`DuplicateKey` outcomes. Clock truth is named at the call site (`Deadline::from_instant(now, dur)`) with a runtime/sim-aware `Context::deadline_after` sugar; no live-only `Deadline::after` shortcut that examples could copy into DST-claimed code. Should land before 067 implementation work begins so the pool-waiter cancellation argument can express its deadline as a `Deadline` rather than a relative `Duration`. |
