...
**Done when:** monoio runtime matches `tina-runtime` on single-shard echo; multi-shard tests prove cross-shard delivery and routing semantics; simulator coverage includes the defined cross-shard model; benchmark results are documented honestly rather than tied to a speculative numeric target.
