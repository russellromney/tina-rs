...
| **085 race / join helpers** | Tina-shaped equivalents for the useful parts of `select!`, `join!`, and `JoinSet`: bounded named call groups, first-success race with visible loser cancellation, join-all with partial deadline report, `RequestContext` integration, and no hidden retry or anonymous branch outcomes. This builds on 081 request contexts, 072 pending call sets, 079 cancellation, and ideally 084 child refs. |
...
| **RequestContext helper polish** | Tiny helper, no full phase unless it grows: add `reply_with_current_request(ctx, f)` on call/observed-call builders as sugar for `ctx.take_request_context()` plus `reply_with_request(...)`. The helper consumes only the current typed request capability; `Context` must stay runtime facts and one-shot capabilities, not app storage or dependency injection. Tests: live + sim success, and loud failure/panic when used on a send-shaped message. Docs should still show the expanded form once so users know where caller authority lives. |
