...
| **100 compile-time safety rails** | Move common silent failures left: split send-only vs callable messages/capabilities where practical, require/diagnose callable handlers, improve trait/macro errors, make cancelable deferred admission harder to order wrong, and use narrow typestate internally where it prevents real protocol bugs. Runtime still owns `Full`/`Closed`/`Timeout` truth. Plan: `.intent/phases/100-compile-time-safety-rails/plan.md`. |
