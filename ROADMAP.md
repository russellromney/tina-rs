...
| **114 Post-113 framework ergonomics** | Add user-job names over the proven workflow primitives: `SharedWork` over `WaitList`, clearer `CancelableWork` / `PendingCancelableCallSet` guidance, bridge-author copy path, system rewrites, compile-fail rails, and stale findings cleanup. Plan: `.intent/phases/114-post-113-framework-ergonomics/plan.md`. |
