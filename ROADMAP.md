...
| **092 AWS bridge follow-ups** | Build the next useful AWS surfaces after S3 proved the bridge shape: SQS send/receive/delete with visibility timeout truth, DynamoDB get/put/query with item-size caps, SNS publish, and Secrets Manager get-secret. Keep fake-local CI, caller-owned retry/idempotency, typed errors, and close/drain truth. Plan: `.intent/phases/092-aws-bridge-followups/plan.md`. |
| **091 timer vocabulary** | Add replay-safe interval, backoff, retry-delay, debounce, and throttle helper state around existing runtime `sleep(...)`. Helper decides delay; user still returns `sleep(delay).reply(...)`. No hidden scheduler, no ambient clock, no invisible dropped work. Plan: `.intent/phases/091-timer-vocabulary/plan.md`. |
| **056 native HTTP/2 service stack** | Second-form HTTP after HTTP/1 body/chunked/cancellation semantics are boring. Adopt sync codec crates where they exist; Tina owns sockets, flow control, stream state, bounded queues, DST, and trace. Not a full RFC/tonic clone. Plan: `.intent/phases/056-native-http2-service-stack/plan.md`. |
