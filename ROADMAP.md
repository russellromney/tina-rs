...
| **059 Eiffel actionable ergonomics** | Turn the current Eiffel findings into product improvements: typed isolate result waiters, continuation/pipeline sugar, TCP loop helpers, capacity diagnostics and reply-slot budgets, tiny native HTTP routing, bridge specimen cleanup, RPC service topology, and a small pressure-report convention. Convenience may remove ceremony, but it may not hide capacity, timeout, topology, retry, `Full`, `Closed`, or trace truth. |
