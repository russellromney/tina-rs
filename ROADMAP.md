...
| **Cancellation round 2** | Continuation of 066. 066 shipped the cancellation/deadline vocabulary; round 2 applies it to surfaces that 066 didn't reach. **Includes deferral from 074:** the chunked response source today is left idle when the wire dies (failure visible only via `body_io_error_count`); this phase lands `ResponseChunkMsg::Cancel` plus the outbound capability the connection isolate needs to send it, so a source can release file handles, downstream RPCs, and other state when the wire is abandoned. Other reachable surfaces (bridge cancellation propagation, supervised-child cancel-on-stop, pool-acquire cancel during shutdown) get audited here for the same shape. The point is consistency: every surface that can hold a resource long enough to outlive its caller should accept a typed cancel. |
| **071 capacity modeling and shared budgets** | Generalize the per-surface pressure counters that 074 shipped (`BodyMetrics` / `BodyPressureReport`) into a shared report shape that other surfaces (mailbox, pending replies, worker pools, bridge queues) can adopt without re-inventing. Defines `CapacitySurfaceReport`, optional shard-local shared scopes, capacity assertion helpers for tests, and a tuning workflow ("unknown -> measured -> fixed"). **Includes deferral from 074:** migrating `BodyPressureReport` into the shared shape, plus the live-tick / periodic emit ergonomics. Snapshot is already callable from any thread; this phase decides whether a generic capacity-tick channel is worth the surface. |
| **HTTP body chunked symmetric** | Client-side chunked decoding plus server-side chunked **request** bodies. 074 shipped server-emit only: server can produce `Transfer-Encoding: chunked` responses, but the HTTP/1 client still rejects chunked responses as `UnsupportedTransferEncoding`, and chunked uploads are still rejected as `UnsupportedTransferEncoding` (501) at the parser. This phase adds: a small chunked decoder behind `parse_response_head`, request-side chunked decoding with the same per-chunk pull contract as response-side, integration tests that exercise both halves, and updates to the streaming docs. **Groups two deferrals from 074 into one slice** because they share decoder code and test scaffolding; splitting them would duplicate the harness. |
...
|---|---|---|
| Cancel surface | **Cancellation round 2** | Connection-to-source cancel signal on wire failure (`ResponseChunkMsg::Cancel`). Today the source is left idle; failure visible only via `body_io_error_count`. 066 shipped the cancellation vocabulary; round 2 applies it to surfaces 066 didn't reach, including this one. |
| Capacity report | **071 capacity modeling** | Migrating `BodyPressureReport` into the shared `CapacitySurfaceReport` shape. Periodic / live-tick metric emit. Both depend on the shared shape landing first. |
| Chunked symmetric | **HTTP body chunked symmetric** | Client-side chunked decoding (today rejected as `UnsupportedTransferEncoding`). Server-side chunked **request** bodies (today rejected). One slice because they share decoder + harness. |
