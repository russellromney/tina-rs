...
| **111 Service product surface** | Assemble existing health/readiness/topology/capacity/shutdown/replay-fact pieces into one copied service report path. No web framework, no global registry, no fake health. Refresh the system specimens that prove a real service can expose pressure, lifecycle, and shutdown truth in one place. Plan: `.intent/phases/111-service-product-surface/plan.md`. |
| **112 Protocol facts to replay** | Phase 103/108 follow-up. Plumb protocol facts that today live as bounded counters on `Http2ConnectionReport`, `BodyMetrics`, `WebSocketMemberTableReport`, and `GrpcStatus` through runtime/simulator trace truth so they can be projected and replayed. Required: stable trace hashes that do not churn for unrelated events, one protocol-fact replay proof, and fail-closed unsupported-fact handling. Plan: `.intent/phases/112-protocol-facts-to-replay/plan.md`. |
| **113 Bridge author kit** | Make bridge writing/review boring: shared lifecycle/pressure/classifier vocabulary, AWS bridge core extraction, one non-AWS bridge aligned to the same language, and author docs that keep worker-terminal vs caller-observed truth honest. No bridge framework blob. Plan: `.intent/phases/113-bridge-author-kit/plan.md`. |
| **114 Post-113 framework ergonomics** | Digest phases 110-113 into one copied service path before the next core wave: skeleton, prelude/import guidance, noun guide, system rewrites, stale findings cleanup, and cheap-model proof. Plan outline: `.intent/phases/114-post-113-framework-ergonomics/plan.md`. |
| **115 Native protocol client parity** | Wave A. Native HTTP/2 and gRPC client isolates, streaming client modes, TLS ALPN, authority/SNI/Host truth, pooled client connections, received-status protocol facts, and interop tests. Plan outline: `.intent/phases/115-native-protocol-client-parity/plan.md`. |
| **116 Local I/O, codec, and IPC parity** | Wave A. Bounded file streaming, line/length-delimited codecs, sync codec adapter pattern, Unix-domain socket rails, local IPC specimens, and sim/unsupported truth. Plan outline: `.intent/phases/116-local-io-codec-ipc-parity/plan.md`. |
| **117 Production resource and data maturity** | Wave A. Pool idle eviction/max lifetime/health/retire rules, pool shutdown reports, pooled DB/HTTP/gRPC clients, snapshot/journal restore patterns, and crash/corrupt-tail recovery specimens. Plan outline: `.intent/phases/117-production-resource-data-maturity/plan.md`. |
| **118 Post-Wave-A ergonomics** | Digest Wave A into the copied service skeleton: outbound protocol clients, file/codec/local IPC, mature pools, durable restore, updated docs/findings, and cheap-model proof. Plan outline: `.intent/phases/118-post-wave-a-ergonomics/plan.md`. |
| **119 Fairness and load behavior** | Wave B. Scheduler/session fairness proofs, timer fairness under hot load, protocol session fairness, remote inbound drain fairness, lag-ish reports, soak/load harnesses, and CPU/memory constrained runs. Plan outline: `.intent/phases/119-fairness-load-behavior/plan.md`. |
| **120 Admission and rate policy** | Wave B. Explicit service pressure policies: concurrency/per-key/per-user/rate limits, bounded wait, shed/degrade/close, retry budget, typed outcomes, capacity/service-report integration, and edge-service specimens. Plan outline: `.intent/phases/120-admission-rate-policy/plan.md`. |
| **121 Ecosystem hooks and async boundary** | Wave B. Public hooks for capacity surfaces, bounded event sinks, sync codecs, service policies, bridge authors, extension smoke crates, runtime capability reports, and explicit native/bridge/unsupported async boundary docs. Plan outline: `.intent/phases/121-ecosystem-hooks-async-boundary/plan.md`. |
