    ...
    S: Shard,
{
        ...
        Outbound: 'static,
    {
        ...
        if let Err(error) = inbox.push(boxed, self.step_ordinal, None) {
    ...
    /// Advances virtual monotonic time by `by`, saturating at
    /// [`Duration::MAX`] instead of wrapping or panicking.
    pub fn advance_time(&mut self, by: Duration) {
        self.virtual_now = self.virtual_now.saturating_add(by);
    }
    ...
    pub fn advance_to_next_timer(&mut self) -> bool {
        let Some(next_deadline) = self
            ...
            .chain(self.call_table.min_isolate_deadline())
            .min()
        else {
            return false;
        };
        ...
        S: 'static,
    {
        ...
        for &index in &dispatch_order {
            ...
            let now = tina::Deadline::from_instant(self.virtual_anchor, self.virtual_now).instant();
    ...
    pub(crate) fn supervise_failed_child(
        ...
        let virtual_now =
            tina::Deadline::from_instant(self.virtual_anchor, self.virtual_now).instant();
        let budget_state = match budget_state.record_restart_at(virtual_now) {
            Ok(next) => next,
            Err(error) => {
                self.push_event(
                    parent,
    ...
    pub(crate) fn dispatch_backend_call(
        ...
        match request {
            CallInput::Sleep { after } => {
                ...
                let after = tina::Deadline::from_instant(self.virtual_anchor, after)
                    .remaining_or_zero(self.virtual_anchor);
                let deadline =
                    self.virtual_now
                        .saturating_add(after)
                        .saturating_add(self.fault_delay(
                            self.config.faults.timer_wake,
                            insertion_order,
                            0x5449_4d45,
                        ));
            ...
            CallInput::TcpWriteOwned {
                ...
                bytes,
                start,
            } => self.handle_tcp_write_owned(call_id, stream, bytes, start),
            ...
            CallInput::FileWriteAtOwned {
                ...
                offset,
                start,
            } => self.handle_file_write_at_owned(call_id, file, bytes, start, offset),
            ...
            CallInput::UnixWriteOwned {
                ...
                bytes,
                start,
            } => self.handle_unix_write_owned(call_id, stream, bytes, start),
            ...
            match route_remote(
                ...
                Ok(()) => {
                    ...
                    self.call_table.insert_isolate(PendingIsolateCall {
                        ...
                        requester: context.requester,
                        cause: context.cause,
                        deadline: self.virtual_now.saturating_add(
                            tina::Deadline::from_instant(self.virtual_anchor, timeout)
                                .remaining_or_zero(self.virtual_anchor),
                        ),
                        insertion_order,
                        continuation_context: context.continuation_context,
    ...
    pub(crate) fn handle_file_write_at(
        ...
        self.schedule_tcp_completion(
            ...
            CallOutput::FileWrote { count },
        );
    }

    pub(crate) fn handle_file_write_at_owned(
        &mut self,
        call_id: CallId,
        file: tina_runtime::FileId,
        bytes: Vec<u8>,
        ...
        let Some(index) = self.file_index(file) else {
            self.deliver_completion(
                call_id,
                CallOutput::FileWroteOwnedFailed {
                    bytes,
                    error: CallError::InvalidResource,
                },
            );
        ...
        self.schedule_tcp_completion(
            call_id,
            TcpResourceKey::FileWrite(file),
            CallOutput::FileWroteOwned { bytes, count },
        );
    }
    ...
    pub(crate) fn handle_unix_write_owned(
        &mut self,
        call_id: CallId,
        stream: UnixStreamId,
        bytes: Vec<u8>,
        ...
        let Some(stream_index) = self.unix_stream_index(stream) else {
            self.deliver_completion(
                call_id,
                CallOutput::UnixWroteOwnedFailed {
                    bytes,
                    error: CallError::InvalidResource,
                },
            );
        ...
        let Some(peer_id) = self.unix_streams[stream_index].peer else {
            self.schedule_tcp_completion(
                call_id,
                resource,
                CallOutput::UnixWroteOwnedFailed {
        ...
        // read that was already parked on the peer.
        if let Some(idx) = self
            .pending_unix_reads
            .iter()
            .position(|pending| pending.stream == peer_id)
    ...
    pub(crate) fn cancel_pending_isolate_calls_for_owner(
        ...
        // Ascending call-id (== insertion) order; record all cancellations
        // before emitting events, matching the runtime.
        let owned = self
            .call_table
                ...
                Some(raw_call_id) => {
                    ...
                    if stamped_shard != self.shard.id().get() {
                        ...
                        match self.call_table.remove_isolate(call_id) {
                            Some(entry) => {
                                // Dropping `entry` drops its translator.
                                let original_cause = entry.cause;
    ...
    pub(crate) fn deliver_completion_at(
        ...
        // Mirror of the runtime: a completion for a call the table no longer
        // tracks is quarantined (traced, dropped), not panicked. Keeps the
        // simulator's observable behavior identical to the live shard's.
    ...
    pub(crate) fn cancel_backend_calls_for_resource(&mut self, resource: TcpResourceKey) {
        ...
        for call_id in cancelled_call_ids {
            ...
            // Removing the driver entry drops its translator.
            if let Some(call) = self.call_table.remove_driver(call_id) {
                self.push_event(
    ...
    pub(crate) fn cancel_backend_calls_for_requester(&mut self, requester: RegisteredAddress) {
        // Ascending call-id order (== insertion order); matches the runtime.
        for call_id in self.call_table.driver_call_ids_for_requester(requester) {
            ...
            self.cancel_pending_backend_work(call.head.call_id);
            self.push_event(
                call.head.requester.isolate,
                Some(call.head.cause),
                RuntimeEventKind::CallCompletionRejected {
...
pub(crate) fn call_kind(request: &CallInput) -> CallKind {
    match request {
        ...
        CallInput::FileWriteAt { .. } | CallInput::FileWriteAtOwned { .. } => CallKind::FileWriteAt,
        ...
        CallInput::UnixWrite { .. } | CallInput::UnixWriteOwned { .. } => CallKind::UnixWrite,
