...
.PHONY: fmt fmt-check check test loom miri doc clippy portable-runtime-cost perf \
	...
	examples-startup-api-guard examples-shutdown-truth-guard
...
examples-shutdown-truth-guard:
	./scripts/examples_shutdown_truth_guard.sh --self-test
	./scripts/examples_shutdown_truth_guard.sh
...
verify-guards: loom race-surface-guard rail-inventory-guard examples-startup-api-guard examples-shutdown-truth-guard
